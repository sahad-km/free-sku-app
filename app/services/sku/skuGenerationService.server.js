...
export async function executeSkuGenerationRun({
  ...
  let startSeqNum = parseInt(skuConfiguration.startNumber, 10) || 1;
  const incrementStep = parseInt(skuConfiguration.incrementStep, 10) || 1;

  if (isSequential && !existingRunId) {
    if (skuConfiguration.bodyNumberType === "continue") {
      const currentSeq = await getCurrentSequenceNumber({
        shopDomain,
        counterKey: "default",
        startNumber: 1,
      });
      startSeqNum = currentSeq;
    }

    await getNextSequenceNumber({
      shopDomain,
      counterKey: "default",
      startNumber: startSeqNum,
      incrementStep,
      count: totalVariants,
    });
  }
