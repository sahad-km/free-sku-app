...
export async function uploadJsonlToStagedUrl(stagedTarget, jsonlContent) {
  ...
  // Get the key parameter to pass as stagedUploadPath to bulkOperationRunMutation
...
export async function pollBulkOperationStatus({ admin, bulkOperationId, runId, totalVariants }) {
  ...
  let attempts = 0;
  const maxAttempts = 600; // max 60 minutes polling interval
  ...
  while (!isCompleted && attempts < maxAttempts) {
    ...
    // Update real-time progress for live polling UI
...
export async function executeShopifyBulkOperationRun({
  ...
  // 1. Traverse product selection and generate JSONL lines
  const jsonlLines = [];
  const auditRows = [];
  ...
  // 4. Trigger bulkOperationRunMutation
